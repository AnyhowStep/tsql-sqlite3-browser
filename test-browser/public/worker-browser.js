...
(function () {
    ...
    const me = getGlobal();
    me.btoa = me.btoa || function (string) {
    ...
    me.atob = me.atob || function (string) {
    ...
    */
    ...
    Database.prototype["create_function"] = function create_function(name, options, func) {
        ...
        function wrapped_func(cx, argc, argv) {
            ...
            setFunctionResult(name, cx, result);
        }
...
// {{PRE_LIBRARY}}
  ...
  var ___tm_current=62368;


  var ___tm_timezone=(stringToUTF8("GMT", 62416, 4), 62416);
...
function initWorker(postMessage) {
    ...
    const processMessage = async (data) => {
        ...
        switch (data.action) {
            ...
            case SqliteAction.CREATE_GLOBAL_JS_FUNCTION: {
                ...
                getGlobal()[data.functionName] = impl;
                postMessage({
                    id: data.id,
                    action: data.action,
                });
            ...
            case SqliteAction.CREATE_FUNCTION: {
                ...
                const impl = eval("(" + data.impl + ")");
                db.create_function(data.functionName, data.options, impl);
                postMessage({
                    id: data.id,
                    action: data.action,
                });
...
onmessage = initWorker(postMessage);
